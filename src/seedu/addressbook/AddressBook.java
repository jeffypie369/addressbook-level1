...
public class AddressBook {
    ...
    private static final String COMMAND_EDIT_PHONE_WORD = "editphone";
    private static final String COMMAND_EDIT_PHONE_DESC = "Edits the phone number belonging to the person.";
    private static final String COMMAND_EDIT_PHONE_PARAMETERS = "NAME "
            + PERSON_DATA_PREFIX_PHONE + "PHONE_NUMBER";
    private static final String COMMAND_EDIT_PHONE_EXAMPLE = COMMAND_EDIT_PHONE_WORD + " name number";
    ...
    private static String PERSON_PROPERTY_NUMBER = "number";
    ...
    /**
     * Edits the phone number of the required person
    ...
    private static String executeEditPhone(String commandArgs) {
        ...
        // edits the person's phone number as specified
        editPersonPhone(person, info.get(1));
        ArrayList<HashMap<String, String>> personInList = new ArrayList<>();
        personInList.add(person);
        showToUser(personInList);
        return "";
    }

    /**
     * Checks if the command is valid
     * @param rawArgs arguments of the command
     * @return true if the command is valid
     */
    private static boolean isEditPersonPhoneArgsValid(String rawArgs) {
        try {
            ArrayList<String> args = extractKeywordsFromEditPersonPhoneArgs(rawArgs.trim());
            String phone = args.get(1);
            return Integer.parseInt(phone) > 0;
    ...
    /**
     * Extracts the words from the command
     * @param editPersonPhoneCommandArgs arguments of the command
     * @return the commands in an ArrayList
     */
    private static ArrayList<String> extractKeywordsFromEditPersonPhoneArgs(String editPersonPhoneCommandArgs) {
        return splitByWhitespace(editPersonPhoneCommandArgs.trim());
    }
    ...
    /**
     * Edits the phone number belonging to the person
     * @param person the actually person inside the address book
     * @param newPhone the new phone number
     */
    private static void editPersonPhone(HashMap<String, String> person, String newPhone) {
        person.replace(PERSON_PROPERTY_NUMBER, newPhone);
    }
    ...
    private static String getPhoneFromPerson(HashMap<String, String> person) {
        return person.get(PERSON_PROPERTY_NUMBER);
    }
...
//        return person;
    ...
    private static boolean isPersonDataValid(HashMap<String, String> person) {
        return isPersonNameValid(person.get(PERSON_PROPERTY_NAME))
                && isPersonPhoneValid(person.get(PERSON_PROPERTY_NUMBER))
    ...
    private static String getUsageInfoForEditPhoneCommand() {
        return String.format(MESSAGE_COMMAND_HELP, COMMAND_EDIT_PHONE_WORD, COMMAND_EDIT_PHONE_DESC) + LS
                + String.format(MESSAGE_COMMAND_HELP_PARAMETERS, COMMAND_EDIT_PHONE_PARAMETERS) + LS
